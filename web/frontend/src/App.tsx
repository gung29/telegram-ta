...
export default function App() {
  ...
  const handleRunTest = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTestResult(
        `Hate: ${formatPercent(result.prob_hate)} • Non-hate: ${formatPercent(result.prob_nonhate)} • Prediksi: ${result.label}`,
      );
    } catch (error) {
  ...
  const StatusCard = () => (
    <div className="status-group">
      <div className="status-card">
        ...
        <div>
          <strong>{autoRefresh ? (loading ? "Realtime sinkronisasi" : "Auto-Protection") : "Mode manual"}</strong>
    ...
    <div className="section-stack">
      ...
      <div className="panel history-panel">
        ...
        <div className="history-charts">
                ...
                options={{
                  ...
                  scales: {
                    y: {
                      beginAtZero: true,
                      suggestedMax: 100,
                      ticks: { callback: (value) => `${value}%`, color: "#e2e8f0" },
          ...
          {visibleHistoryEvents.slice(0, 2).map((event) => {
            const meta = ACTION_META[event.action] ?? { label: event.action, tone: "muted" };
            return (
              <div key={event.id} className="history-chart action">
                <h4>{meta.label}</h4>
                <p className="muted">{event.text ?? "-"}</p>
                <small>{dayjs(event.created_at).fromNow()}</small>
              </div>
            );
          })}
        </div>
  ...
  const AdminView = () => (
    <div className="section-stack">
      ...
      <div className="panel user-actions-panel">
        <div className="panel-header">
          <div>
        ...
        <div className="user-actions-grid">
          {userActions.length === 0 && !userActionsLoading && <p className="muted">Belum ada data pengguna untuk ditampilkan.</p>}
      ...
      <div className="panel members-panel">
        <ModerationForm
          title="Pengguna dimute"
          status="muted"
          data={muted}
          onSubmit={handleAddMemberStatus}
          onRelease={handleRemoveMember}
        />
        <ModerationForm
          ...
          <tbody>
            ...
            {data.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  {item.expires_at
                    ? dayjs(item.expires_at).format("DD MMM HH:mm")
                    : status === "muted"
                    ? "Sampai dibuka"
                    : "Permanent"}
                </td>
